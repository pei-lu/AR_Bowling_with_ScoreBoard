...
namespace Qualcomm.Snapdragon.Spaces
{
    ...
    public sealed partial class ImageTrackingFeature : SpacesOpenXRFeature
    {
        ...
        private Dictionary<string, Dictionary<uint, ulong>> _trackablesNameAndIdToHandle = new Dictionary<string, Dictionary<uint, ulong>>();
        ...
        public bool TryCreateImageTracker(out ulong imageTrackerHandle, RuntimeReferenceImageLibrary imageLibrary, int maxNumberOfMovingImages) {
            ...
            SpacesRuntimeReferenceImageLibrary spacesImageLibrary =
                (SpacesRuntimeReferenceImageLibrary) imageLibrary;
            ...
            IntPtr targetModesPtr = Marshal.AllocHGlobal(targetCount * Marshal.SizeOf<int>());
            Marshal.Copy(targetModes, 0, targetModesPtr, targetCount);

            XrImageTargetsTrackingModeInfoQCOM modeInfo =
                new XrImageTargetsTrackingModeInfoQCOM(targetCount, targetNamesArrayPtr, targetModesPtr);

            IntPtr modeInfoPtr = Marshal.AllocHGlobal(Marshal.SizeOf<XrImageTargetsTrackingModeInfoQCOM>());
            Marshal.StructureToPtr(modeInfo, modeInfoPtr, false);
            ...
            if (_xrSetImageTargetsTrackingModeQCOM == null) {
	            ...
	            createInfo = new XrImageTrackerCreateInfoQCOM(dataSetPtr, (uint)targetCount, (uint)maxNumberOfMovingImages, modeInfoPtr);
            }
            ...
            Marshal.FreeHGlobal(targetModesPtr);
        ...
        public bool TrySetTrackingModes(ulong imageTrackerHandle, List<string> referenceImageNames, List<SpacesImageTrackingMode> trackingModes) {
	        ...
	        var modeInfo = new XrImageTargetsTrackingModeInfoQCOM(targetCount, targetNamesArrayPtr, targetModesPtr);
	        IntPtr modeInfoPtr = Marshal.AllocHGlobal(Marshal.SizeOf<XrImageTargetsTrackingModeInfoQCOM>());
	        Marshal.StructureToPtr(modeInfo, modeInfoPtr, false);
        ...
        public bool TryStopTrackingImageInstance(string referenceImageName, uint id) {
	        ...
	        ulong imageTargetHandle = _trackablesNameAndIdToHandle[referenceImageName][id];
