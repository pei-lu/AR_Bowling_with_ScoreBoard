...
namespace Qualcomm.Snapdragon.Spaces
{
    public class SpacesRuntimeReferenceImageLibrary : RuntimeReferenceImageLibrary
    {
        ...
        private readonly SpacesReferenceImageTrackingModes _trackingModes = new SpacesReferenceImageTrackingModes();
        ...
        internal SpacesRuntimeReferenceImageLibrary(XRReferenceImageLibrary serializedLibrary, SpacesReferenceImageTrackingModes trackingModes) {
	        ...
	        for (int i = 0; i < serializedLibrary.count; i++) {
                ...
                if (!trackingModes.ReferenceImageNames.Contains(serializedLibrary[i].name)) {
                    ...
                    _trackingModes.AddTrackingModeForReferenceImage(serializedLibrary[i].name, XrImageTargetTrackingModeQCOM.XR_IMAGE_TARGET_TRACKING_MODE_DYNAMIC_QCOM);
                }
                else {
	                _trackingModes.AddTrackingModeForReferenceImage(serializedLibrary[i].name, trackingModes.GetTrackingModeForReferenceImage(serializedLibrary[i].name));
                }
        ...
        internal SpacesReferenceImageTrackingModes TrackingModes => _trackingModes;
    }
}
