...
namespace Qualcomm.Snapdragon.Spaces
{
    ...
    internal sealed partial class HitTestingFeature : SpacesOpenXRFeature
    {
        ...
        private static List<XRRaycastSubsystemDescriptor> _raycastSubsystemDescriptors = new List<XRRaycastSubsystemDescriptor>();

        protected override string GetXrLayersToLoad() {
            return ""; // should not be tied to a specific layer - the extension can come from multiple locations
        }
        ...
        protected override void OnSubsystemCreate() => CreateSubsystem<XRRaycastSubsystemDescriptor, XRRaycastSubsystem>(_raycastSubsystemDescriptors, RaycastSubsystem.ID);

        protected override void OnSubsystemStop() => StopSubsystem<XRRaycastSubsystem>();

        protected override void OnSubsystemDestroy() => DestroySubsystem<XRRaycastSubsystem>();
        ...
        public bool TryGetRayCastCollisions(ulong rayCastHandle, Vector3 origin, out List<XRRaycastHit> raycastHits) {
            ...
            for (int i = 0; i < collisionsCapacityInput; i++) {
                ...
                raycastHits.Add(new XRRaycastHit(new TrackableId(rayCastHandle, rayCollision.TargetId), new Pose(position, Quaternion.LookRotation(rayCollision.SurfaceNormal)), Vector3.Distance(position, origin), TrackableType.Planes));
            }
