...
namespace Qualcomm.Snapdragon.Spaces
{
    ...
    [OpenXRFeature(
        ...
        FeatureId = FeatureID)]
#endif
    public sealed partial class PlaneDetectionFeature : SpacesOpenXRFeature
    {
        ...
        private static Dictionary<ulong, PlaneDataCollection> _planesDataMap =
            ...
            for (int planeIx = 0; planeIx < (int) scenePlaneCount; ++planeIx) {
                ...
                if (!ConvexHullEnabled) {
                    if (vertexList.Count >= 3 && indexList.Count >= 3) {
                        ...
                        var v2 = vertexList[(int)indexList[2]]; // next two indices flipped because of winding order changes
            ...
            if (ConvexHullEnabled) {
                ...
                for (int indexIx = 0; indexIx < indexBuffer.Count; ++indexIx) {
                    ...
                    if (vertexIx >= vertexBuffer.Count) {
                        Debug.LogWarning(
                            $"Cannot add vertex with index {vertexIx} because the vertex buffer only contains {vertexBuffer.Count} vertices");
                        continue;
                    }
